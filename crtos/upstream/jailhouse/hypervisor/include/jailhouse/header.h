...
struct jailhouse_header {
	...
	/** Offset of bootstrap_vectors from hypervisor link base (ARM64).
	 * @note Filled at build time. */
	unsigned long bootstrap_vectors_off;
	/** Offset of el2_entry from hypervisor link base (ARM64, Pi5 SOFT_RESTART).
	 * @note Filled at build time. */
	unsigned long el2_entry_off;
	/** EL1 virt scratch page (Pi5 debug); same phys as 0x1ffb0000.
	 * @note Filled by driver when scratch_trace=1. */
	void *scratch_trace_base;
};
