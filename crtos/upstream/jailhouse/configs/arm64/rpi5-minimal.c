/*
 * Minimal Jailhouse system config for Raspberry Pi 5 bring-up.
 * No virtual PCI / IVSHMEM — smallest path to test hypervisor enable.
 *
 * DT overlay: jailhouse-crtos-mem (hv @ 0x1fc00000, inmate hole @ 0x20000000)
 */
...
#define RPI5_AXI_MMIO_SIZE	0x10000000ULL	/* 256MiB — same as rpi5.c draft */
...
struct {
	...
	.mem_regions = {
		...
		/* AXI: RP1, PCIe, IOMMU (256MiB — smaller EL2 PT vs 1GiB) */
		{
		...
		/* RAM low (below hv/shmem/inmate hole) */
		{
			.phys_start = 0x0,
			.virt_start = 0x0,
			.size = 0x1fa10000,
		...
		/* RAM 512M..4G (matches upstream rpi4 layout; inmate hole via DT nomap) */
		{
