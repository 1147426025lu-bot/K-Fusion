# ============================================================================
# plc_fusion_common__公共库.sh — K-Fusion 脚本公共库（source 使用，勿直接执行）
# ============================================================================
...
_REPO_PATHS="$_COMMON_DIR/../lib/repo_paths__仓库路径.sh"
if [ -f "$_REPO_PATHS" ]; then
    # shellcheck source=../lib/repo_paths__仓库路径.sh
    source "$_REPO_PATHS"
fi
...
plc_project_root() {
    if [ -n "${KFUSION_ROOT:-}" ] && [ -f "${KFUSION_ROOT}/CMakeLists.txt" ]; then
        echo "$KFUSION_ROOT"
        return
    fi
    local dir start
    ...
    if [ "$(basename "$start")" = "scripts" ] || [ "$(basename "$start")" = "fuse" ]; then
        cd "$start/.." && pwd
        return
    fi
    echo "$start"
...
# 内核化 Pass 插件路径（KFusionPass.so 优先，兼容 PLCFusionPass.so）
plc_fusion_pass_so() {
    local root="${1:-$(plc_project_root)}"
    local build="${2:-$root/build}"
    if [ -f "$build/KFusionPass.so" ]; then
        echo "$build/KFusionPass.so"
    elif [ -f "$build/PLCFusionPass.so" ]; then
        echo "$build/PLCFusionPass.so"
    else
        echo "$build/KFusionPass.so"
    fi
}

plc_fusion_pass_target() {
    if [ -f "$(plc_project_root)/build/KFusionPass.so" ] || \
       [ -f "$(plc_project_root)/build/PLCFusionPass.so" ]; then
        if [ -f "$(plc_project_root)/build/KFusionPass.so" ]; then
            echo "KFusionPass"
        else
            echo "PLCFusionPass"
        fi
...
plc_remap_hint_for_sym() {
    case "$1" in
        ...
        vfprintf) echo plc_fprintf ;;
        vprintf) echo plc_printk ;;
        vsnprintf) echo plc_snprintf ;;
        fflush) echo plc_fflush ;;
        fsync) echo plc_fsync ;;
        pthread_detach) echo plc_pthread_detach ;;
        pthread_attr_setdetachstate|pthread_attr_setinheritsched|\
        pthread_attr_setschedpolicy) echo "plc_${1}" ;;
        sem_init|sem_destroy|sem_wait|sem_post|sem_timedwait|sem_getvalue) echo "plc_${1}" ;;
        sched_getparam|sched_getscheduler) echo "plc_${1}" ;;
...
# LLC 产物若含 .llvm_bss，重命名为 .bss 以便内核链接（freestanding .o）
plc_fuse_merge_llvm_bss() {
    local obj="$1"
    local llc_arch="${2:-}"
    local objcopy
    [ -f "$obj" ] || return 0
    objcopy="$(plc_resolve_objcopy "$llc_arch")"
    if ! "$objcopy" --section-headers "$obj" 2>/dev/null | grep -q '\.llvm_bss'; then
        return 0
    fi
    "$objcopy" --rename-section .llvm_bss=.bss,alloc,write,contents "$obj" 2>/dev/null \
        || plc_warn "无法合并 .llvm_bss → .bss: $obj" "可设 FUSE_MERGE_LLVM_BSS=0 跳过"
}
