...
根目录下大量 `*.ll` / `*.validate.json` 属 GENERATED，可安全删除后重建。**`*.ko` 保留**（论文 W5 测量 preflight 依赖 `test/plc_multitask_mod.ko` 等）。

```bash
bash scripts/maintenance/cleanup_repo_local__本地仓库清理.sh
```

测量 W5 时 `results/paper/multitask/.multitask.lock` 存在 — 清理脚本会跳过 multitask 归档；勿在矩阵运行中手动删 `.ko` 或 `plc_multitask_uspace`。
