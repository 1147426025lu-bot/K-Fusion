...
export RING_EXPORT_PATH="${RING_EXPORT_PATH:-$REPO/results/jitter_8h_soak.bin}"

STAMP="$(date +%Y%m%d_%H%M%S)"
LOG="$REPO/results/soak/stability/soak_8h_${STAMP}.log"
MONITOR_NOTE="$REPO/results/soak/stability/soak_8h_${STAMP}.monitor.txt"
mkdir -p "$REPO/results/soak/stability"
