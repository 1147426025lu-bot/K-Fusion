...
- **扩写稿 + Table II/III + Figure 7**：见 [`COMPARE_KTC_vs_PLCFUSION__KTC与PLCFusion对照.md`](COMPARE_KTC_vs_PLCFUSION__KTC与PLCFusion对照.md)。
...
| Timed C / KTC | 语言+CIL，**用户态** | 无 | POSIX/FreeRTOS | WCET+sched+jitter | **Pi** |

**编译器级对照（Table II）与 Pi 三基线协议（Table III）** 见同上对照文档；**Figure 7** 为 KTC vs PLCFusion 双流水线图。
...
| W5 | plc_multitask（可选） | STRICT LET 多 rate + 定点 + 多 TU（单线程时间复用，非 pthread worker） |
...
### 9.6 第二应用与泛化
- signaltest insmod 短测；plc_multitask STRICT LET 多 TU（可选一节，见 `MULTITASK_EVAL__多任务评估.md`）。  
